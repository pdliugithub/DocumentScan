package com.rossia.life.scan.tensor.tracking;
...
import com.rossia.life.scan.tensor.env.Logger;
 ...
 *         <p>
 *         Object detection tracker box.
 *         </p>
 */
public class MultiBoxTracker {

    private static final String TAG_LOG = "MultiBoxTracker";
    ...
    /**
     * 当绘制Box边框完成后的Callback
     */
    private OnDrawRectCompleteCallback mOnDrawRectCompleteCallback;

    /**
     * 当为检测出对象时Callback.
     */
    private DetectionNothingCallback mDetectionNothingCallback;
    /**
     * 这里存储了绘制的颜色值
     */
    ...
    private final Queue<Integer> mAvailableColors = new LinkedList<Integer>();
     ...
     * 检测出的对象在屏幕上展示的Rect区域
     */
    final List<Pair<Float, RectF>> mScreenRects = new LinkedList<Pair<Float, RectF>>();
    ...
    public synchronized void drawDebug(final Canvas canvas) {
        ...
        for (final Pair<Float, RectF> detection : mScreenRects) {
        ...
        mObjectTracker.drawDebug(canvas, matrix);
    }
    ...
    public synchronized void onFrame(
        ...
        if (mObjectTracker == null && !initialized) {
            ObjectTracker.clearInstance();

            logger.i("Initializing ObjectTracker: %dx%d", w, h);
            mObjectTracker = ObjectTracker.getInstance(w, h, rowStride, true);
            frameWidth = w;
            frameHeight = h;
...
//                Toast.makeText(context, message, Toast.LENGTH_LONG).show();
        ...
        mObjectTracker.nextFrame(frame, null, timestamp, null, true);

        // Clean up any objects not worth tracking any more.
        final LinkedList<TrackedRecognition> copyList =
                new LinkedList<TrackedRecognition>(mTrackedObjects);
        for (final TrackedRecognition recognition : copyList) {
            ...
            if (correlation < MIN_CORRELATION) {
                ...
                mTrackedObjects.remove(recognition);

                mAvailableColors.add(recognition.color);
            }
        }
    }
    ...
    private void processResults(
        ...
        for (final Classifier.Recognition result : results) {
            ...
            mScreenRects.add(new Pair<Float, RectF>(result.getConfidence(), detectionScreenRect));
...
//            return;
        ...
        /*
        PS:貌似 mObjectTracker 一直为null
         */
        if (mObjectTracker == null) {
            LogUtil.e(TAG_LOG, "objectTracker is null:\t " + (mObjectTracker == null));
            ...
            for (final Pair<Float, Classifier.Recognition> potential : rectsToTrack) {
                ...
                trackedRecognition.color = COLORS[mTrackedObjects.size()];
                mTrackedObjects.add(trackedRecognition);

                if (mTrackedObjects.size() >= COLORS.length) {
                    break;
                }
            }
            return;
    ...
    private void handleDetection(
        ...
        final ObjectTracker.TrackedObject potentialObject =
                mObjectTracker.trackObject(potential.second.getLocation(), timestamp, frameCopy);
        ...
        for (final TrackedRecognition trackedRecognition : mTrackedObjects) {
            ...
            //如果矩形a和b相交，则返回true，并将该矩形设置为该交点，否则返回false并且不更改此矩形。不执行检查以查看任一矩形是否为空。为了测试相交，使用intersects（）
        ...
        // object.
        if (mAvailableColors.isEmpty() && removeList.isEmpty()) {
            for (final TrackedRecognition candidate : mTrackedObjects) {
        ...
        for (final TrackedRecognition trackedRecognition : removeList) {
            ...
            mTrackedObjects.remove(trackedRecognition);
            if (trackedRecognition != recogToReplace) {
                mAvailableColors.add(trackedRecognition.color);
            }
        }

        if (recogToReplace == null && mAvailableColors.isEmpty()) {
        ...
        trackedRecognition.color =
                recogToReplace != null ? recogToReplace.color : mAvailableColors.poll();
        mTrackedObjects.add(trackedRecognition);
    }

    public void setOnDrawRectCompleteCallback(OnDrawRectCompleteCallback onDrawRectCompleteCallback) {
    ...
    /**
     * 此类绘制完成所识别的边框后Callback
     */
    public interface OnDrawRectCompleteCallback {
        /**
         * 绘制Rect矩形区域完成后的Callback.
         *
         * @param rectF {@link RectF}
         * @return null
         */
